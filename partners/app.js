  ...
  if (typeof web3 !== "undefined") {
    ...
    let netVersion = web3.eth.net.getId().then(async function (data) {
      if (data == "56") {
        ...
        // inputAddress.value = "Please switch to Binance Smart Chain network";
  ...
  async function safuLock(
    ...
    sendBtn.addEventListener('click', () => {
      ...
      safuContract.methods
        .lockTokens(tokenAddress, userAmount, timeStamp, false)
        .send({
          from: userAddress
        })
        .on('transactionHash', () => {
        ...
        .on('receipt', () => {
          ...
          successNotification.innerHTML = `You have successfully locked <span style = "color: #f2a900">${userAmount / (10** decimals)}</span> <span style = "color: #00ab94">${symbol}</span>. <br> <a href="https://twitter.com/intent/tweet?text=There%20are%20${userAmount / (10 ** decimals)}%20${symbol}%20tokens%20locked%20in%20@safulock%20smart%20contract%20until%20${userDate}." target = "_blank">Share your lock on<i class="fab fa-twitter"></i>Twitter</a>`;
        })
        .on('error', (e) => {
          alert('Error');
          console.log(e);
